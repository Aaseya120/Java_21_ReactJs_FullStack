...
/**
 * Outbox Relay Scheduler. Polls the outbox_events table for unprocessed events
 * and publishes them to Kafka. This guarantees at-least-once delivery even if
 * the message broker is temporarily down.
 */
@Component
	...
	public void processOutboxEvents() {
		...
		for (OutboxEvent event : events) {
			try {
				...
				kafkaTemplate.send(KafkaConstants.TOPIC_ORDER_EVENTS, event.getAggregateId(), event.getPayload()).get();
